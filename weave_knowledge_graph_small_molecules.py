...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        metavar="TSV",
        nargs="+",
        help="Extract from the Omnipath networks TSV file, subsetting the small molecule protein interactions.",
    )
    ...
    if asked.small_molecules:
        logging.info(f"Weave Omnipath small molecule protein networks data...")

        small_molecules_df = pd.read_csv(asked.small_molecules[0], sep="\t")
        print(small_molecules_df.info())

        mapping_file = "./omnipath_secondary_adapter/adapters/small_molecules.yaml"
        with open(mapping_file) as fd:
            mapping = yaml.full_load(fd)
        ...
        logging.info(f"Wove small molecule protein Networks: {len(nodes)} nodes, {len(edges)} edges.")
